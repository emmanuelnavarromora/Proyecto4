...
# ---
# 
# * Estudiante: **Emmanuel Navarro Mora**
# * Carné: **B65024**
# * Grupo: **2**
# 
# ---
...
def moduladorI(bits, fc, mpp):
    ...
    # 5. Calcular la potencia promedio de la señal modulada
    PmI = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    
    return senal_TxI, PmI, portadoraI, moduladoraI  
...
def moduladorQ(bits, fc, mpp):
    ...
    # 5. Calcular la potencia promedio de la señal modulada
    PmQ = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    
    return senal_TxQ, PmQ, portadoraQ, moduladoraQ  
...
def modulador(bits, fc, mpp,portadoraQ,portadoraI):
    # 1. Parámetros de la 'señal' de información (bits)
    N = len(bits) # Cantidad de bits
    ...
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)
    portadora = portadoraI+portadoraQ

    # 3. Inicializar la señal modulada s(t)
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_TxT = np.zeros(t_simulacion.shape)
    moduladora = np.zeros(t_simulacion.shape)  # señal de información
 
    # 4. Asignar las formas de onda según los bits (BPSK)
    for i, bit in enumerate(bits):
        if bit == 1:
            senal_TxT[i*mpp : (i+1)*mpp] = portadora
            moduladora[i*mpp : (i+1)*mpp] = 1
        else:
            senal_TxT[i*mpp : (i+1)*mpp] = portadora * -1
            moduladora[i*mpp : (i+1)*mpp] = 0
    
    # 5. Calcular la potencia promedio de la señal modulada
    Pm = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    
    return senal_TxT, Pm, portadora, moduladora  
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_TxT, Pm, portadora, moduladora = modulador(bits_Tx, fc, mpp,portadoraQ,portadoraI)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_TxT, Pm, SNR)
...
# La señal modulada por BPSK
ax2.plot(senal_TxT[0:600], color='g', lw=2) 
ax2.set_ylabel('$s(t)$')
...
# Analizamos las cuatro formas de onda posible
wo = 2* np.pi*fc
T = 100			# número de elementos
t_final = 10	# tiempo en segundos
t = np.linspace(0, t_final, T)
sen1=1*np.cos(wo*t)+1*np.sin(wo*t)
plt.plot(sen1)
print('La media de la senal 1 es:',np.mean(sen1))

sen2=-1*np.cos(wo*t)+1*np.sin(wo*t)
plt.plot(sen2)
print('La media de la senal 2 es:',np.mean(sen2))

sen3=1*np.cos(wo*t)+((-1)*np.sin(wo*t))
plt.plot(sen3)
print('La media de la senal 3 es:',np.mean(sen3))

sen4=((-1)*np.cos(wo*t))+((-1)*np.sin(wo*t))
plt.plot(sen4)
print('La media de la senal 4 es:',np.mean(sen4))


# Podemos aproximar que la media de la amplitud para las cuatro ondas es cero
...
for i in range(N):
    w_t = vaA*senal_TxT[0:200]
    W_t[i,:] = w_t
    plt.plot(t, w_t)

sigsig= np.var(senal_TxT)
sigmean= np.mean(senal_TxT)
sigsquare=math.pow(sigmean, 2)
...
# Si existe o no estacionaridad depende de si se considera cierta tolerancia o no, ya que se sabe que el promedio de la amplitud es 0, asi tambien el promedio de senal_Tx es 0, si se saca el promedio para cada instante de t es un número muy cercano a 0, cambia hasta el tercer decimal la mayoria de las veces, si esta aproximación es lo suficientemente buena entonces se cumple por estacionaridad de primer orden que E[X(t1 + ∆)] = E[X(t1)], por lo que se puede decir que es un proceso estacionario
# 
